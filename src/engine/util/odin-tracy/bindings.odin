...
// Must match how TracyClient was built. vendor/CMakeLists.txt always enables
// TRACY_ON_DEMAND, so this repo defaults it to true. With ON_DEMAND enabled the
// C side adds a uint64_t connectionId member to TracyCZoneCtx, which changes
// both the return convention (hidden sret pointer) and the by-value argument
// passing of every zone function. An 8-byte Odin struct there corrupts the
// call stack the moment a connected profiler starts emitting real zones.
TRACY_ON_DEMAND :: #config(TRACY_ON_DEMAND, true)

when ODIN_OS == .Darwin  do foreign import tracy "tracy.dylib"
when ODIN_OS == .Windows do foreign import tracy "../../../../vendor/build/_deps/tracy-build/TracyClient.lib"
...
when TRACY_ON_DEMAND {
	// TracyCZoneCtx gains a uint64_t connectionId member when the client is
	// compiled with TRACY_ON_DEMAND. Leaving it out changes the struct size
	// (8 vs 16 bytes), which flips the Windows x64 calling convention for the
	// zone functions from register-passed to hidden-pointer-return/argument.
	// That corrupts the call stack as soon as a connected profiler emits real
	// zones, so the layout must match how TracyClient was built.
