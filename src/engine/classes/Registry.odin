...
Registry_Init :: proc(
	...
	return Registry{datatypes = datatype_registry, enums = enum_registry, renderer = renderer, data_model = data_model}
}
...
descriptor_get :: proc(L: ^vm.State, value, ctx: rawptr, key: string) -> bool {
	...
	if descriptor != nil && descriptor.get != nil && descriptor.get(L, object, descriptor.registry.datatypes, descriptor.registry.enums, key) {
		return true
	}
...
descriptor_set :: proc(L: ^vm.State, value, ctx: rawptr, key: string, value_index: int) -> bool {
	descriptor := cast(^Class_Descriptor)ctx
	object := cast(^Object)value
	if descriptor != nil && descriptor.set != nil && descriptor.set(L, object, descriptor.registry.datatypes, descriptor.registry.enums, key, value_index) {
		return true
	}
...
descriptor_namecall :: proc(L: ^vm.State, value, ctx: rawptr, method: string) -> (i32, bool) {
	...
	if descriptor != nil && descriptor.namecall != nil {
		result_count, handled := descriptor.namecall(L, object, descriptor.registry.datatypes, descriptor.registry.enums, method)
		if handled {
...
Push_New :: proc(registry: ^Registry, vm_state: ^vm.VM, class_name: string, require_creatable := true) -> (^Object, bool) {
